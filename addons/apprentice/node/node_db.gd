...
func _remove_child_data(node: Node) -> void:
	if _program_is_exiting or not is_instance_valid(self) or not is_instance_valid(node):
		return
	
	if record_all_child and node.tree_exiting.is_connected(self._remove_child_data):
