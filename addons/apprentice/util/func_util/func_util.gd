...
static func for_recti(rect: Rect2i, callable: Callable) -> void:
	for y in range(rect.position.y, rect.end.y + 1):
		for x in range(rect.position.x, rect.end.x + 1):
...
##  路径移动（广度优先搜索）。一般用于搜索路径，比如用在 [TileMap] 从开始一个坐标开始搜索周围没有瓦片的坐标
##[br]
...
static func path_move(
	...
	while true:
		...
		if last.size() == 0:
			break
		if begin_next_callback.is_valid():
			begin_next_callback.call()
...
static func ready_call(parent: Node, callback: Callable) -> void:
...
static func combination(items: Array) -> Array[Array]:
	...
	# 执行方法
	var callable : Array = []
	callable.append(func(tmp: Array, from: int, max_idx: int):
		tmp.append(from)
		result.append(tmp.map(func(item_index): return items[item_index] ))
		
		from += 1
		for i in range(from, max_idx):
			callable[0].call(tmp.duplicate(), i, max_idx)
...
##[/codeblock]
static func sync_property(callable: Callable) -> void:
	var from := callable.get_object() as Node
