...
static func get_four_directionsi() -> Array[Vector2i]:
	return [Vector2i.LEFT, Vector2i.UP, Vector2i.RIGHT, Vector2i.DOWN]

static func get_eight_directionsi() -> Array[Vector2i]:
...
class RandomProbabilityGenerator:
	...
	func _init(params: Dictionary):
		value_list = params.keys()
		probability_list = params.values()
...
##})
##[/codeblock]
static func random_probability(param: Dictionary):
	return RandomProbabilityGenerator.new(param).rand_value()
...
## 获取两个值的中心
static func get_center(from, to):
