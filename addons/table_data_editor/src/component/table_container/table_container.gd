...
## 新增单元格
signal newly_added_cell(coordinate: Vector2i, new_cell: Control)
## 网格中的单元格大小发生改变
signal grid_tile_size_changed(grid_size: Vector2i)
...
func _ready() -> void:
	...
	# cell 的更新与大小
	_row_container.newly_added_line.connect(_newly_added_line)
	...
	(func():
		...
		_update_row_column_amount_timer.timeout.connect(func():
			...
			if tmp_cell_amount != _grid_cell_count_size:
				...
				grid_tile_size_changed.emit(_grid_cell_count_size)
...
#  连接信号
#============================================================
func _newly_added_line(new_line: ColumnContainer):
	new_line.newly_added_cell.connect( func(new_cell: Control): 
		self._cell_list.append(new_cell)
		
		var column = new_cell.get_index()
		var row = new_line.get_index()
